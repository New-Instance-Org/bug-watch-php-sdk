...
final class BugWatch
{
    ...
    {
        return self::client()->withSpan($name, $fn, $options);
    }

    public static function setTraceContext(?string $traceId, ?string $spanId = null): void
    {
        self::client()->setTraceContext($traceId, $spanId);
    }

    /** @return array<string,string> */
    public static function traceHeaders(): array
    {
        $ctx = self::client()->getTraceContext();
        $header = TraceContext::buildTraceparent($ctx['traceId'], $ctx['spanId']);

        return $header === null ? [] : ['traceparent' => $header];
    }
